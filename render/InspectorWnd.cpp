...
BOOL CInspectorWnd::Create(CWnd* parent_wnd) {
    ...
    m_toggleTriangulationLinesButton.Create(_T("삼각분할 선: OFF"),
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
...
int CInspectorWnd::LayoutButtons(int start_y) {
    int y = start_y;
...
void CInspectorWnd::OnPaint() {
    ...
    int legend_y = LayoutButtons(y + 10);   // 버튼들을 먼저 배치하고, 그 아래 y를 받아옴
    ...
    DrawLevel(&mem_dc, legend_y, width);    // 버튼들보다 아래에 범례를 그림
    mem_dc.SelectObject(old_font);
...
void CInspectorWnd::OnToggleTriangulationLinesClicked() {
    ...
    m_toggleTriangulationLinesButton.SetWindowText(
        m_showTriangulationLines ? _T("삼각분할 선: ON") : _T("삼각분할 선: OFF"));
